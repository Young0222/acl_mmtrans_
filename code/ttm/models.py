...
class PLMPlain(nn.Module):
    def __init__(self, opt):
        ...
        ql_tokens = {
            'AQL': ['FOR', 'IN', 'INBOUND', 'OUTBOUND', 'FILTER', '==', 'AND', 'CONTAINS', 'COLLECT', 'WITH', 'COUNT', 'INTO', 'SORT', 'DESC', 'ASC', 'LIMIT', 'RETURN', 'LET'],
            'ECQL': ['@>', 'JSONB', 'ARRAY', 'ELEMENTS', 'LENGTH', '{', '}', 'MAX'],
            'SQL++': ['inE', 'outE', 'inV', 'outV', 'class'],
            'MIR': ['TableScan', 'OrderBy']
        }

        additional_tokens = [AddedToken(' ' + s) for s in list(set([word for words in ql_tokens.values() for word in words]))]

        
        self.tokenizer.add_tokens([AddedToken(" <="), AddedToken(" <")] + additional_tokens)
    ...
    def prepare_schema(self):
        with open(config.SCHEMA_PATH, 'r', encoding='utf-8') as fj:
